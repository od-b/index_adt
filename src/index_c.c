...
void index_addpath(index_t *index, char *path, list_t *words) {
    ...
    while ((curr_word = list_next(words_iter)) != NULL) {
        /* skip any duplicate words from within the same path */
        if (!set_contains(new_i_path->i_words_at_path, (char*)curr_word)) {

            /* check if index map contains the word already */
            i_word_t *curr_i_word = map_get(index->i_words, curr_word);
